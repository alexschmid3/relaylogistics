...
function solvejourneymodel(lprelax_flag, opt_gap, orderarcs, numeffshifts)
	...
	#Objective
	@objective(ip, Min, lambda * sum((ordtime[i] - shortesttriptimes[i])/shortesttriptimes[i] for i in orders) + sum(sum(c[a]*x[i,a] for a in orderarcs.A[i]) for i in orders) + sum(c[a]*(y[a] ) for a in hasdriverarcs.A) + sum(u[a]*(w[a] ) for a in A_space) )
	...
	#Linking constraints
	@constraint(ip, driverAvailability[a in primaryarcs.A_space], sum(sum(sum(z[l,s,f] for f in fragmentscontaining[l,s,a]) for s in 1:numeffshifts) for l in 1:numlocs) == w[a]  )
