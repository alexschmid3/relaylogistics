...
function columngeneration!(orderarcs, hasdriverarcs)
	...
	#Linking constraints
	@constraint(rmp, driverAvailability[a in primaryarcs.A_space], sum(sum(sum(z[l,s,f] for f in fragmentscontaining[l,s,a]) for s in 1:numshifts) for l in 1:numlocs)  == w[a] )
