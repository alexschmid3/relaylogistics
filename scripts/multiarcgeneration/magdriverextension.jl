...
function sparsemasterproblem_dext(orderArcSet, orderArcSet_space, A_plus_i, A_minus_i, homeArcSet, A_minus_d, A_plus_d, availableDrivers)
	...
	#Objective
	@objective(smp, Min, lambda * sum((ordtime[i] - shortesttriptimes[i])/shortesttriptimes[i] for i in orders) + sum(sum(c[a]*x[i,a] for a in orderArcSet[i]) for i in orders) + sum(c[a]*y[a] for a in A_hasdriver) + sum(u[a]*w[a] for a in A_space) + lambda2 * maxhours)
...
function dextmultivariablegeneration!(orderArcSet, orderArcSet_space, A_plus_i, A_minus_i, bpRestrictedArcSet, homeArcSet, A_minus_d, A_plus_d, availableDrivers, A_minus_i_full, A_plus_i_full)
