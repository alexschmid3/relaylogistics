...
function multiarcgeneration_heterogeneous!(magarcs, hasdriverarcs, startercuts, starterfixedvars, variableusecount, currvarfixingiter, cg_iter_start, maxcgiterations)
	...
	elseif deadlines_flag == 1
		@objective(smp, Min, lambda * sum(orderdelay[i] for i in orders) + sum(sum(c[a]*x[i,a] for a in magarcs.A[i]) for i in orders) + sum(c[a]*y[a] for a in hasdriverarcs.A) + sum(u[a]*w[a] for a in primaryarcs.A_space) + lambda2 * maxhours)
	...
	while cg_iter <= cg_iter_start + maxcgiterations
        ...
        try
			push!(lowerbounds, smpobj + sum(minreducedcosts))
		...
		elseif (minimum(minreducedcosts) >= -0.0001) & (currvarfixingiter < varsettingiterations) & (cutsaddedthisiter == 0)
            ...
            for fixedvalue in [1,0]
                ...
                obj_fix, smp_fix, x_fix, y_fix, z_fix, w_fix, magarcs_fix, smptm_fix, pptm_fix, pppar_fix, arcs_fix, cgiter_fix, cuts_fix, cuttime_fx, trash1, trash2 = multiarcgeneration_heterogeneous!(magarcs, hasdriverarcs, mastercuts, masterfixedvars, variableusecount, currvarfixingiter+1, cg_iter+1, 25)    
				println("Final iteration was $cgiter_fix")
				cg_iter += - cg_iter + cgiter_fix 
				println("Updated cg_iter to $cg_iter")
				#println("That was node $currvarfixingiter btw")

				#Add times to lists
				smptimes = union(smptimes, smptm_fix)
				pptimes = union(pptimes, pptm_fix)
				pptimes_par = union(pptimes_par, pppar_fix)
				cuttime += cuttime_fx
