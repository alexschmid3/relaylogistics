	...
	while cg_iter <= cg_iter_start + maxcgiterations
		...
		elseif (minimum(minreducedcosts) >= -0.0001) & (currvarfixingiter < varsettingiterations) & (cutsaddedthisiter == 0)
            ...
            for fixedvalue in [1,0]
                ...
                obj_fix, smp_fix, x_fix, y_fix, z_fix, w_fix, magarcs_fix, smptm_fix, pptm_fix, pppar_fix, arcs_fix, cgiter_fix, cuts_fix, cuttime_fx, trash1, trash2 = multiarcgeneration_heterogeneous!(magarcs, hasdriverarcs, mastercuts, masterfixedvars, variableusecount, currvarfixingiter+1, cg_iter+1, 10)    
